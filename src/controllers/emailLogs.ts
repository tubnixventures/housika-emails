...
export async function listEmailLogs(
  token: string,
  options: { page?: number; perPage?: number } & EmailLogFilter = {}
  ...
  const total = Number((countRow.rows?.[0] as any)?.count ?? 0);

  const listParams = [...params, perPage, offset];
  const rows = await db.execute(
    `SELECT id, sender_role, sender_email, recipient_email, subject, body, sent_at, created_at
     FROM email_logs
     ${whereSql}
