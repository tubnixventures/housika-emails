...
if (!bunnyUrl) {
  throw new Error(
    "Missing Bunny DB URL: set BUNNY_DATABASE_URL or BUNNY_DB_URL in your environment"
  );
}
if (!bunnyToken) {
  throw new Error(
    "Missing Bunny DB token: set BUNNY_DB_TOKEN (or authToken) in your environment"
  );
}
...
export async function logEmailAudit(opts: {
  ...
  await db.execute(
    `INSERT INTO email_logs (id, sender_role, sender_email, recipient_email, subject, body, sent_at, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?);`,
    [uuidv4(), opts.senderRole, opts.senderEmail, opts.recipientEmail, opts.subject, opts.body, sentAt, now]
  );
}
